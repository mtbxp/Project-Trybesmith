...
const validationTypes = (req: Request, res: Response, next: NextFunction) => {
  const { username, vocation, level, password } = req.body;
...
const validationLength = (req: Request, res: Response, next: NextFunction) => {
  const { username, vocation, level, password } = req.body;
...
export default { validationFields, validationTypes, validationLength };